...
function play_atari(c::Chromosome, id::String, seed::Int64;
    ...
    while ~game_over(game.ale)
        ...
        if (frames % step_size) == 0
            ...
            if 0.5 * max_frames < frames && stop_playing_check(reward, frames)
    ...
    actions_used = count(x->x>0, outputs)
    println("Actions used: ", actions_used)
    ...
    if length(values(max_reward_dict)) < 1
        max_reward_dict = score_dict
        println("Updated the local top fit!")
    elseif reward > maximum(values(max_reward_dict))
        max_reward_dict = score_dict
        println("Updated the local top fit!")
    end

    # Return the reward and the list of outputs
    reward, outputs, reward_with_regularization
end
...
function render_genes(genes::Array{Float64}, args::Dict;
    ...
    Logging.info(string(out_counts))
    ...
    file =  string("graphs/", args["id"], "_", args["seed"], "_", id, "_action.pdf");
...
if ~isinteractive()
    ...
    # Set up logging
    Logging.configure(filename=string(args["id"], "_action.log"), level=INFO)
